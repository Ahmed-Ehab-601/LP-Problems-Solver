...
class GoalProgramming(Solver):
    def SetLinearProblem(self):
       ...
       if self.LP.needBigM :
          currentZ += 1
          self.LP.known_variables["M"] = 100*self.LP.m  
    ...
    def solve(self):
       ...
       for i in self.LP.goal_values:
          ...
          if(self.LP.state == "optimal" or self.LP.state == "Degeneracy"):
               if(self.LP.needBigM and zIndex == 0):
...
def test():
    ...
    inputtest1 = Input( #sat g1 and constrains 
        n=2, m=6,
        constraints=[
    ...
    inputtest2 = Input(  # can't satisfy constrains 84/27,30/17
        n=2, m=6,
        constraints=[
    ...
    inputtest3 = Input( #sat g1 and constrains  3,5
        n=2, m=6,
        constraints=[
            Constrain([7, 3], "<=", 40, priority=6,isGoal=True),
            Constrain([10, 5], "=", 60, priority=5,isGoal=True),
    ...
    inputtest4 = Input( #sat g2 and constrains  5/3,5 but check can satisfy is wrong 
        n=2, m=6,
        constraints=[
            Constrain([7, 3], "<=", 40, priority=6,isGoal=True),
            Constrain([10, 5], "=", 60, priority=5,isGoal=True),
   ...
   #      goal.solve()
    goal = GoalProgramming(inputtest4)
    goal.SetLinearProblem()   
    goal.solve()
